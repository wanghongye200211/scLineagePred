# -*- coding: utf-8 -*-
"""
GSE175634 - Regression training in GENE (log) space, following the SAME workflow as your GSE99915 script.

Core method (unchanged):
- Use genes.txt from autoencoder results as the output gene list.
- Use sequence H5 that MUST contain `indices` so we can fetch real gene expression from adata by index.
- Random split over sequences (DeepLineage-style), no clone split.
- Base models: RNN / BiLSTM / Transformer
- Per-gene ridge stacking on VAL
- Optional sign-fix patch (clone-mean correlation) to avoid rare anti-correlated genes

This version includes:
- Transformer positional encoding + proper key padding mask + masked pooling
- MPS fix: disable nested tensor fastpath in TransformerEncoder (enable_nested_tensor=False)
- Memory-safe expression loading for large datasets:
  keep adata.X sparse if possible, densify ONLY VAL/TEST subsets when needed.
...
class Config:
    # ---- dataset-specific ----
    OUT_PREFIX = "GSE175634_CMvsCF_all_generated"

    # ===== paths =====
    ae_result_dir = "/Users/wanghongye/python/scLineagetracer/autoencoder/results/GSE175634"   # must contain genes.txt
    time_series_h5 = ""   # auto from OUT_PREFIX in main()
    index_csv = ""        # auto from OUT_PREFIX in main()

    # columns in index_csv
    index_clone_col = "clone_id"   # if not found, will try fallbacks
    csv_label_col = "label_str"    # if not found, will try fallbacks (e.g. 'type')

    # NOTE: your adata file
    adata_h5ad = "/Users/wanghongye/python/scLineagetracer/GSE175634/processed/GSE175634_with_latent_and_clone.h5ad"
    out_dir = "/Users/wanghongye/python/scLineagetracer/regression/result/GSE175634"
    ...
    # tasks mode:
    #   "all_prev_only"  -> predict LAST timepoint using ALL previous inputs (1 task, 99915 style)
    ...
    # Optional: if sequences.h5 contains a mask dataset (N,T) of 0/1,
...
def get_expr_matrix(adata, source: str):
    """
    Memory-safe:
    - Keep sparse if possible (CSR float32)
    - Dense only if already dense.
    """
    if source == "X":
        X = adata.X
    elif source == "raw":
...
def take_rows_dense(X_expr, rows: np.ndarray) -> np.ndarray:
    rows = np.asarray(rows, dtype=np.int64)
    if sp.issparse(X_expr):
        return np.asarray(X_expr[rows].toarray(), dtype=np.float32)
    return np.asarray(X_expr[rows], dtype=np.float32)
...
class DirectPredictor(nn.Module):
    ...
    def __init__(self, kind: str, in_dim: int, out_dim: int, hidden: int, dropout: float):
        ...
        if kind == "RNN":
            ...
            # MPS: avoid nested tensor path
            self.net = nn.TransformerEncoder(enc, num_layers=2, enable_nested_tensor=False)
            enc_out = d_model
        else:
...
class RegDataset(Dataset):
    def __init__(
        ...
        X_expr,  # dense ndarray or sparse csr
    ):
    ...
    def __getitem__(self, i):
        tgt_i = int(self.tgt[i])
        if sp.issparse(self.X_expr):
            y = np.asarray(self.X_expr[tgt_i].toarray(), dtype=np.float32).ravel()
        else:
            y = np.asarray(self.X_expr[tgt_i], dtype=np.float32).ravel()

        y = torch.from_numpy(y)
        if self.kpm is None:
            return self.X[i], None, y, self.clone[i], self.lbl[i], tgt_i
        return self.X[i], self.kpm[i].to(torch.bool), y, self.clone[i], self.lbl[i], tgt_i
...
def pick_first_existing_col(df: pd.DataFrame, preferred: str, fallbacks):
...
def main():
    ...
    # auto paths from OUT_PREFIX
    cfg.time_series_h5 = f"/Users/wanghongye/python/scLineagetracer/GSE175634/processed/{cfg.OUT_PREFIX}_sequences.h5"
    cfg.index_csv = f"/Users/wanghongye/python/scLineagetracer/GSE175634/processed/{cfg.OUT_PREFIX}_index.csv"
    ...
    # ---------- genes ----------
    genes_full = read_gene_list(cfg.ae_result_dir)

    # ---------- adata (for real gene expression) ----------
    print(f"[Data] Loading adata: {cfg.adata_h5ad}")
    adata = ad.read_h5ad(cfg.adata_h5ad)

    # align genes robustly (drop missing from genes.txt)
    adata_genes = set(adata.var_names.astype(str))
    genes = [str(g) for g in genes_full if str(g) in adata_genes]
    missing = [str(g) for g in genes_full if str(g) not in adata_genes]
    if missing:
        print(f"[WARN] genes.txt has {len(missing)} genes not found in adata.var_names; dropped. example={missing[:10]}")
    genes = np.array(genes, dtype=object)
    if genes.size == 0:
        raise ValueError("After gene alignment, 0 genes remain. Check genes.txt vs adata.var_names.")

    adata = adata[:, genes].copy()
    X_expr = get_expr_matrix(adata, cfg.adata_expr_source)
    print(f"[Expr] source={cfg.adata_expr_source} shape={X_expr.shape} sparse={sp.issparse(X_expr)}")
    ...
    if indices is None:
        raise ValueError(
            ...
            "This script follows the 99915 method and requires indices.\n"
    ...
    # ---------- index CSV (labels + clone ids) ----------
    df_idx = pd.read_csv(cfg.index_csv)
    if len(df_idx) != N:
        raise ValueError(f"index_csv rows ({len(df_idx)}) != sequences ({N}). Use matching files.")

    # robust column picks
    clone_col = pick_first_existing_col(df_idx, cfg.index_clone_col, fallbacks=["clone_id_ruot", "clone", "clonal_id"])
    if clone_col is None:
        raise KeyError(f"Cannot find clone id column. tried={ [cfg.index_clone_col,'clone_id_ruot','clone','clonal_id'] }")
    clone_ids_seq = df_idx[clone_col].astype(str).values

    label_col = pick_first_existing_col(df_idx, cfg.csv_label_col, fallbacks=["type", "label", "cell_type", "endpoint"])
    if label_col is not None:
        label_str_seq = df_idx[label_col].astype(str).values
    else:
        if label_str_h5 is None:
            raise KeyError(f"Missing label column in CSV and label_str in H5.")
        label_str_seq = np.array([
            (x.decode("utf-8") if isinstance(x, (bytes, np.bytes_)) else str(x)).strip()
            for x in label_str_h5
        ], dtype=object)

    print(f"[IndexCSV] clone_col={clone_col} label_col={label_col if label_col else 'H5(label_str)'}")
    ...
    for task_name, in_pos, tgt_pos in cfg.tasks:
        ...
        keep = np.isin(label_str_seq[valid0], np.array(cfg.keep_labels, dtype=object))
        valid = valid0[keep]
        if valid.size == 0:
        ...
        out_dim = len(genes)  # regression in gene space
        for name in base_names:
            print(f"[Train] {name}")
            model = DirectPredictor(name, in_dim=X_in.shape[-1], out_dim=out_dim, hidden=cfg.hidden, dropout=cfg.dropout)
        ...
        y_val = take_rows_dense(X_expr, tgt_cell_idx[idx_va])
        ...
        y_true = take_rows_dense(X_expr, meta["tgt"])
