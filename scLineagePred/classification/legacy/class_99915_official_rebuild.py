...
# ================= 配置区域 =================
class Config:
    time_series_h5 = "/Users/wanghongye/python/scLineagetracer/GSE99915/processed/GSE99915_DeepLineage_Official_CloneGroup_sequences.h5"
    index_csv = "/Users/wanghongye/python/scLineagetracer/GSE99915/processed/GSE99915_DeepLineage_Official_CloneGroup_index.csv"

    out_dir = "/Users/wanghongye/python/scLineagetracer/classification/GSE99915_official_rebuild"
...
def load_data(cfg):
    ...
    with h5py.File(cfg.time_series_h5, "r") as f:
        ...
        mask_all = np.array(f["mask"], dtype=np.float32) if "mask" in f else None
    ...
    # Use missing-timepoint mask as an explicit input channel.
    if cfg.use_mask_channel and mask_all is not None:
        m_bin = mask_all[keep_indices].astype(np.float32)[..., None]
        X_bin = np.concatenate([X_bin, m_bin], axis=2)

    print(f"Loaded {len(X_bin)} samples. ({pos} vs {neg})")
    return X_bin, y_bin, clones_bin


def get_split(n_samples, seed, y):
...
def main():
    ...
    for setting, mask in settings.items():
        ...
        tr_idx, va_idx, te_idx = get_split(len(X), current_seed, y)
