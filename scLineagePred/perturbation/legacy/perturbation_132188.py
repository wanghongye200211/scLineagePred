# -*- coding: utf-8 -*-
"""
GSE132188 - Perturbation Scan (4-class, 15.5-focused)
=======================================================

Reference style: Downstream_114412/Perturbation.py (multi-class)

This script only keeps two perturbation scenarios requested by user:
1) Observe up to 14.5 (UpTo_14.5), perturb first 3 timepoints (t0,t1,t2), predict endpoint classes at 15.5.
2) Observe up to 15.5 (All_15.5), perturb first 4 timepoints (t0,t1,t2,t3), predict endpoint classes at 15.5.
...
<out_dir>/
  perturbation/
    <scenario_tag>/
      dose_response_all_dims.csv
      dim_summary.csv
      t0_xxx/dose_response_all_dims.csv
      t0_xxx/dim_summary.csv
      ...
    dim_summary_all_targets.csv
  downstream/
    genes_transition_*csv
    decoder_<scenario>__<target>/top_genes_per_dim.csv
    decoder_<scenario>__<target>/marker_genes_ranked_<class>.csv
    marker_gene_candidates_union.csv
    driver_genes_master_<class>.csv
    driver_genes_<class>.csv
  driver_genes_master_<class>.csv
  driver_genes_<class>.csv
  run_config.json
"""
...
class Config:
    # ---------- dataset ----------
    OUT_PREFIX: str = "GSE132188_DeepLineage_all_generated"
    processed_dir: str = "/Users/wanghongye/python/scLineagetracer/GSE132188/processed"
    ...
    # endpoint classes (must match class_132188.py)
    TARGET_ENDPOINT_CLUSTERS: Tuple[str, ...] = ("Alpha", "Beta", "Delta", "Epsilon")
    csv_label_col: str = "label_str"

    # ---------- classifier ensemble ----------
    model_dir: str = "/Users/wanghongye/python/scLineagetracer/classification/GSE132188/saved_models"
    base_seed: int = 2026
    ...
    # ---------- decoder ----------
    decoder_dir: str = "/Users/wanghongye/python/scLineagetracer/autoencoder/results/GSE132188"
    genes_txt: str = field(init=False)
    z_genes_npy: str = field(init=False)

    hvg_h5ad: str = "/Users/wanghongye/python/scLineagetracer/GSE132188/processed/GSE132188_with_latent.h5ad"
    ...
    # ---------- outputs ----------
    out_dir: str = "/Users/wanghongye/python/scLineagetracer/Downstream_132188/Perturbation"
...
def load_sequences(cfg: Config):
    ...
    if not keep.any():
        raise ValueError(f"[ERROR] No sequences match TARGET_ENDPOINT_CLUSTERS={class_names}")
...
# ===================== Strategy =====================
def build_15p5_focused_strategies(time_labels: List[str]):
    """
    Build two fixed strategies:
...
def main():
    ...
    strategies = build_15p5_focused_strategies(time_labels)
    print("[INFO] Active 15.5-focused strategies:")
    for s in strategies:
        print(
    ...
    for k in ["TARGET_ENDPOINT_CLUSTERS", "folds"]:
        if k in run_meta and isinstance(run_meta[k], tuple):
            run_meta[k] = list(run_meta[k])
