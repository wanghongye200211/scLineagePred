# -*- coding: utf-8 -*-
"""
GSE99915 - Regression training in GENE (log) space, following the same workflow as your GSE140802 "Part 1".

Key points (same as 140802):
- Use genes.txt from autoencoder results as the HVG list / output genes.
...
IMPORTANT MODIFICATION for MASKED sequences (GSE99915):
- Keep only ("Reprogrammed","Failed") by sequence-level label_str in sequences.h5 (NOT by target-cell adata.obs label).
- Do NOT require all input timepoints present; inputs can be missing (masked).
- Mask-aware normalization (ignore missing=0 positions when computing mu/sd).
...
class Config:
    # ===== paths =====
    ae_result_dir = "/Users/wanghongye/python/scLineagetracer/autoencoder/results/GSE99915"   # must contain genes.txt
    ...
    index_clone_col = "clone_id"   # column in index_csv
    adata_h5ad = "/Users/wanghongye/python/scLineagetracer/GSE99915/processed/GSE99915_final_integrated.h5ad"
    out_dir = "/Users/wanghongye/python/scLineagetracer/regression/result/GSE99915"

    # Expression source in LOG space:
    #   "X" | "raw" | "layer:<name>"
    adata_expr_source = "X"

    # obs column names (for sanity prints only; not used for filtering now)
    adata_label_col = "state_info"
    adata_time_col = "time_info"

    # keep only these two fates (sequence-level label_str)
    keep_labels = ("Reprogrammed", "Failed")

    # timepoints order in sequences (must match how sequences were built)
    timepoints = ["Day6", "Day9", "Day12", "Day15", "Day21", "Day28"]

    # tasks: (name, in_pos, tgt_pos) where positions index into `timepoints`
    tasks = [
        ("Reg_D28_from_D6_D9_D12_D15_D21",  [0, 1, 2, 3, 4], 5),
    ]
...
def load_h5_sequences(path: str):
    ...
    Expected datasets:
      ...
      - label_str: optional bytes/str [N] (sequence-level fate label; used for keep_labels on masked data)
    """
...
def build_loaders(X_in, mask_in, tgt_cell_idx, labels_tgt, clone_ids, X_expr, idx_tr, idx_va, idx_te, batch_size):
    """
    Mask-aware normalization for MASKED sequences.

    - mu/sd are computed ONLY on observed input positions (mask==1) from training set.
    - missing positions are kept at 0 after normalization (do NOT inject fake signal).
    """
    eps = 1e-6
    ...
    if mask_in is None:
        ...
        mu = (X_in[idx_tr] * mtr[:, :, None]).sum(axis=(0, 1)) / (den + eps)  # [D]
        ...
        def _norm(X, m):
            Xn = (X - mu) / sd
            return Xn * m[:, :, None]  # keep missing at 0
...
def fit_stacking(preds_val_list, y_val, alpha):
    ...
    for g in tqdm(range(G), desc="Stacking (per gene)"):
        ...
        A[-1, -1] = 0.0  # do not regularize bias
...
def main():
    ...
    print(f"[Expr] source={cfg.adata_expr_source} min={x_min:.4f} max={x_max:.4f} shape={X_expr.shape}")

    # sequences + indices (+ optional mask + label_str)
    print(f"[Data] Loading sequences: {cfg.time_series_h5}")
    X_seq, indices, mask, label_str = load_h5_sequences(cfg.time_series_h5)
    if indices is None:
        raise ValueError(
            "Your sequences.h5 does NOT contain `indices`, so we cannot fetch real gene expression from adata.\n"
            "This script follows the GSE140802 method and requires indices.\n"
            "Please regenerate sequences with indices saved (same as 140802), or use a strict-decoder evaluation pipeline."
        )
    N, T, D = X_seq.shape
    print(f"[Seq] X_seq={X_seq.shape} indices={indices.shape} mask={'None' if mask is None else mask.shape}")

    # sequence-level fate labels (from Step5); used for keep_labels on masked data
    if label_str is None:
        raise ValueError("Your sequences.h5 does NOT contain `label_str`. Please regenerate Step5 sequences with label_str saved.")
    label_str_seq = np.array([
        (x.decode("utf-8") if isinstance(x, (bytes, np.bytes_)) else str(x)).strip()
        for x in label_str
    ], dtype=object)

    if T != len(cfg.timepoints):
        print(f"[WARN] Sequence T={T} but Config.timepoints has {len(cfg.timepoints)} entries: {cfg.timepoints}")
    ...
    for task_name, in_pos, tgt_pos in cfg.tasks:
        ...
        print(f"\n=== Task: {task_name} | in_pos={in_pos} -> tgt_pos={tgt_pos} ({cfg.timepoints[tgt_pos] if tgt_pos < len(cfg.timepoints) else tgt_pos}) ===")
        tdir = os.path.join(cfg.out_dir, task_name)
        ensure_dir(tdir)
        ensure_dir(os.path.join(tdir, "ckpt"))

        # ===== validity: only require TARGET present =====
        m = (indices[:, tgt_pos] >= 0)

        # For MASKED sequences, inputs can be missing.
        if mask is not None:
            m &= (mask[:, tgt_pos] == 1)
            # at least one observed input timepoint (others can be missing -> already filled with 0 in X_seq)
            m &= (mask[:, in_pos].sum(axis=1) > 0)
        else:
            # if no mask, require at least one input index exists
        ...
        # ===== keep only two fates using SEQUENCE label_str (NOT target-cell state_info) =====
        keep = np.isin(label_str_seq[valid0], cfg.keep_labels)
        valid = valid0[keep]

        if valid.size < 200:
            vc = pd.Series(label_str_seq[valid0]).value_counts().to_dict()
            print(f"[WARN] Too few valid sequences after label_str filter: {valid.size}. keep={cfg.keep_labels} | label_str counts={vc}")

        if valid.size == 0:
            print("[Skip] 0 sequences after label_str filter.")
            continue

        # subset arrays to valid
        X_in = X_seq[valid][:, in_pos, :]                    # [n, T_in, D]
        tgt_cell_idx = indices[valid, tgt_pos].astype(np.int64)

        # For plotting/grouping we use SEQUENCE-level fate label (Reprogrammed/Failed).
        labels_tgt = label_str_seq[valid]

        clone_ids = clone_ids_seq[valid]

        # input mask for normalization (missing inputs should not bias mu/sd)
        if mask is not None:
        ...
        np.savez_compressed(
            ...
            timepoints=np.array(cfg.timepoints, dtype=object),
