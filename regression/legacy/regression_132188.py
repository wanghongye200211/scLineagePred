# -*- coding: utf-8 -*-
"""
GSE132188 - Regression training in GENE (log) space, following the same workflow as GSE114412/GSE175634 scripts.

Compared to regression_114412.py, only dataset-specific items are changed:
- paths (OUT_PREFIX / h5 / csv / h5ad / ae_result_dir / out_dir)
- cell types (keep_labels)
- time information (auto read from sequences.h5)
- reverse-order compatibility for 132188 sequences (from index_csv.samples_order)

In addition, the Transformer (Trans) is upgraded to:
- positional encoding (位置编码 positional encoding)
- proper key padding mask (掩码 mask / padding mask)
- masked pooling (只在有效时间步上做 pooling)
...
class Config:
    ...
    # ===== paths =====
    ae_result_dir = "/Users/wanghongye/python/scLineagetracer/autoencoder/results/GSE132188"   # must contain genes.txt
    time_series_h5 = ""   # auto from OUT_PREFIX
    index_csv = ""        # auto from OUT_PREFIX
    index_clone_col = "clone_id"   # column in index_csv
    csv_label_col = "label_str"    # sequence-level endpoint label

    # NOTE: update this if your integrated h5ad name differs
    adata_h5ad = "/Users/wanghongye/python/scLineagetracer/GSE132188/processed/GSE132188_with_latent_and_clone.h5ad"
    out_dir = "/Users/wanghongye/python/scLineagetracer/regression/result/GSE132188"
    ...
    keep_labels = ("Alpha", "Beta", "Delta", "Epsilon")
...
def main():
    ...
    cfg.time_series_h5 = f"/Users/wanghongye/python/scLineagetracer/GSE132188/processed/{cfg.OUT_PREFIX}_sequences.h5"
    cfg.index_csv = f"/Users/wanghongye/python/scLineagetracer/GSE132188/processed/{cfg.OUT_PREFIX}_index.csv"
