# -*- coding: utf-8 -*-
"""
decoder_175634.py (log1p only, seaborn-template style)
...
class Config:
    ...
    scatter_genes_fail = []     # label=CM
    scatter_genes_repr = []     # label=CF
    violin_genes_fail = []      # CM
    violin_genes_repr = []      # CF

    # (Optional) reasonable defaults if you want manual lists:
    # - CF (fibroblast-like): Col1a1, Col1a2, Col5a2
    # - CM (cardiomyocyte-like): Tnnt2, Actc1, Myh6
...
def plot_scatter(genes, pred_log, true_log, highlight_idx, out_path, title, cfg: Config):
    ...
    mx = mx + max(0.06 * mx, 0.1)  # 顶部留空间（至少 +0.1）
...
def main():
    ...
    for task in tasks:
        ...
        manual_scatter = {"CM": cfg.scatter_genes_fail, "CF": cfg.scatter_genes_repr}
        manual_violin = {"CM": cfg.violin_genes_fail, "CF": cfg.violin_genes_repr}
